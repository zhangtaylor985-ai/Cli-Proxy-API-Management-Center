...
export const SLOW_USAGE_THRESHOLD_MS = 90_000;
    ...
    Object.entries(models).forEach(([modelName, modelEntry]) => {
      ...
      modelDetails.forEach((detailRaw) => {
        ...
        details.push({
          ...
          latency_ms:
            typeof detailRaw.latency_ms === 'number'
              ? detailRaw.latency_ms
              : typeof detailRaw.latency_ms === 'string'
                ? Number(detailRaw.latency_ms) || 0
                : 0,
...
export function usageDetailLatencyMs(detail: UsageDetail): number {
  const value = detail.latency_ms;
  if (typeof value === 'number' && Number.isFinite(value)) return value;
  return 0;
}

export function isSlowUsageDetail(
  detail: UsageDetail,
  thresholdMs: number = SLOW_USAGE_THRESHOLD_MS
): boolean {
  if (detail.failed) return false;
  return usageDetailLatencyMs(detail) >= thresholdMs;
}
